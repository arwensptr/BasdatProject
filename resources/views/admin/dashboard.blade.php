    ...
    <div class="admin-grid section">
        ...
        <div class="section">
            ...
            <div class="flex flex-col xl:flex-row xl:items-center justify-between mb-8 gap-4">
                {{-- Kiri: Navigasi & Filter --}}
                <div class="flex flex-col sm:flex-row sm:items-center gap-3 w-full xl:w-auto">
                    <div class="flex items-center gap-2">
                         <a href="{{ route('admin.orders.index') }}" class="h-10 inline-flex items-center px-4 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 text-sm font-medium rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition-all">
                            Kelola Order
                        </a>
                        <a href="{{ route('admin.medicines.index') }}" class="h-10 inline-flex items-center px-4 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 text-sm font-medium rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition-all">
                            Kelola Stok
                        </a>
                    </div>
                    ...
                    <form action="{{ route('admin.dashboard') }}" method="GET" class="flex flex-wrap items-center gap-2">
                        ...
                        @if(request('bulan') || request('quarter'))
                            <a href="{{ route('admin.dashboard') }}" class="h-10 inline-flex items-center gap-1.5 px-3 bg-red-50 text-red-600 border border-red-200 rounded-xl text-sm font-medium hover:bg-red-100 transition-all dark:bg-red-900/20 dark:border-red-800 dark:text-red-400">
                                Reset
                            </a>
                        @endif
                    </form>
            ...
            <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                ...
                <div class="p-5 flex items-center gap-4 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border-l-4 border-l-blue-500 dark:border-slate-700">
                    ...
                    <div>
                        ...
                        <div class="text-2xl font-bold text-slate-800 dark:text-white">{{ $oltpStats['rx_pending'] ?? 0 }}</div>
                    </div>
                </div>
                ...
                <div class="p-5 flex items-center gap-4 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border-l-4 border-l-amber-500 dark:border-slate-700">
                    ...
                    <div>
                        ...
                        <div class="text-2xl font-bold text-slate-800 dark:text-white">{{ $oltpStats['pay_pending'] ?? 0 }}</div>
                    </div>
                </div>
                ...
                <div class="p-5 flex items-center gap-4 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border-l-4 border-l-blue-500 dark:border-slate-700">
                    ...
                    <div>
                        ...
                        <div class="text-2xl font-bold text-slate-800 dark:text-white">{{ $oltpStats['orders_open'] ?? 0 }}</div>
                    </div>
                </div>
                ...
                <div class="p-5 flex items-center gap-4 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border-l-4 border-l-emerald-500 dark:border-slate-700">
                    ...
                    <div>
                        ...
                        <div class="text-2xl font-bold text-slate-800 dark:text-white">{{ $oltpStats['orders_today'] ?? 0 }}</div>
                    </div>
                </div>
            </div>

            {{-- 2. RINGKASAN KINERJA (Cards - ADAPTIVE WARNA BOX & RAPI) --}}
            <h3 class="text-lg font-bold text-slate-700 dark:text-slate-300 mb-4">
                Ringkasan Kinerja {{ request('quarter') ? request('quarter') : '' }} {{ request('bulan') ? date('F', mktime(0,0,0,request('bulan'),1)) : '' }} (2025)
            </h3>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div class="p-6 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex items-center justify-between transition-colors">
                    ...
                    <div>
                        ...
                        <h4 class="text-3xl font-bold text-slate-800 dark:text-white mt-1">Rp {{ number_format($totalRevenue, 0, ',', '.') }}</h4>
                    ...
                    {{-- Right Side: Icon (Dalam Lingkaran) --}}
                ...
                <div class="p-6 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex items-center justify-between transition-colors">
                    ...
                    <div>
                        ...
                        <h4 class="text-3xl font-bold text-slate-800 dark:text-white mt-1">{{ number_format($totalOrdersDW, 0, ',', '.') }} <span class="text-lg font-normal text-slate-400">Order</span></h4>
                ...
                <div class="p-6 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex items-center justify-between transition-colors">
                    ...
                    <div>
                        ...
                        <h4 class="text-3xl font-bold text-slate-800 dark:text-white mt-1">{{ number_format($totalQtySold, 0, ',', '.') }} <span class="text-lg font-normal text-slate-400">Unit</span></h4>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                {{-- Penjualan --}}
                <a href="{{ route('admin.analytics.sales') }}" class="group p-6 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm hover:border-blue-500 dark:hover:border-blue-500 transition-all cursor-pointer">
                ...
                {{-- Lifecycle --}}
                <a href="{{ route('admin.analytics.lifecycle') }}" class="group p-6 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm hover:border-emerald-500 dark:hover:border-emerald-500 transition-all cursor-pointer">
                    <div class="flex items-center justify-between mb-4">
                ...
                {{-- Obat --}}
                <a href="{{ route('admin.analytics.inventory') }}" class="group p-6 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm hover:border-purple-500 dark:hover:border-purple-500 transition-all cursor-pointer">
                    <div class="flex items-center justify-between mb-4">
