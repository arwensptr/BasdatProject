    ...
    <div class="admin-grid section">
        ...
        <div class="section">
            ...
            <div class="grid sm:grid-cols-3 gap-4">
                <div class="card pad">
                    <div class="text-sm text-gray-500 dark:text-slate-400">Menunggu Review</div>
                    <div class="text-3xl font-bold dark:text-white">{{ method_exists($pending,'total') ? $pending->total() : $pending->count() }}</div>
                </div>
                <div class="card pad">
                    <div class="text-sm text-gray-500 dark:text-slate-400">Disetujui</div>
                    <div class="text-3xl font-bold dark:text-white">{{ isset($approved) && method_exists($approved,'total') ? $approved->total() : ($approved->count() ?? 0) }}</div>
                </div>
                <div class="card pad">
                    <div class="text-sm text-gray-500 dark:text-slate-400">Ditolak</div>
                    <div class="text-3xl font-bold dark:text-white">{{ isset($rejected) && method_exists($rejected,'total') ? $rejected->total() : ($rejected->count() ?? 0) }}</div>
                </div>
            </div>
