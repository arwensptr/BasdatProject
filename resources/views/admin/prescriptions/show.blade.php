<x-app-layout>
    <x-slot name="header">
        <h2 class="font-display text-2xl text-brand-800 dark:text-brand-300">Resep — Order #{{ $prescription->order_id }}</h2>
    </x-slot>
    ...
    <div class="admin-grid section">
        ...
        <div class="section">
            <div class="grid lg:grid-cols-2 gap-6">
                <div class="card pad">
                    ...
                    <div class="text-sm space-y-2 dark:text-slate-300">
                        <div>Pemilik: <b>{{ $prescription->user->name }}</b> <span class="text-gray-500 dark:text-slate-400">({{ $prescription->user->email }})</span></div>
                        ...
                        @if($prescription->note)
                            <div class="p-3 bg-gray-50 rounded-xl dark:bg-slate-700 dark:text-slate-300">Catatan: {{ $prescription->note }}</div>
                        @endif
                    </div>
                </div>
                    ...
                    <table class="min-w-full text-sm table-ui">
                        ...
                        <tbody class="divide-y dark:divide-slate-700">
                            @foreach($prescription->order->items as $it)
                                <tr>
                                    <td class="px-4 py-3 dark:text-slate-300">{{ $it->medicine->name ?? 'Unknown' }}</td>
                                    <td class="px-4 py-3 text-center dark:text-slate-300">{{ $it->qty }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
            ...
            <a class="text-brand-700 underline dark:text-brand-400" href="{{ route('admin.prescriptions.index') }}">← Kembali</a>
        </div>
    </div>
</x-app-layout>
