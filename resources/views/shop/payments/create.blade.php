<x-app-layout>
  <x-slot name="header">
    <div class="flex items-center justify-between">
      <h2 class="text-2xl font-semibold text-gray-800">Upload Bukti Pembayaran — Pesanan #{{ $order->id }}</h2>
      ...
      <div class="bg-white shadow-lg rounded-lg">
        <form method="POST" action="{{ route('shop.payments.store',$order) }}" enctype="multipart/form-data">
          ...
          <div class="p-8">
            ...
            <div>
              <label for="file-upload" class="btn btn-white w-full h48 flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50">
                ...
                <span id="file-name-display" class="text-sm text-gray-500 mt-2"></span>
              </label>
          ...
          {{-- Tombol Aksi --}}
          <div class="bg-gray-50 px-6 py-6 items-center justify-between gap-4 rounded-b-lg">
            <a href="{{ route('shop.orders.show',$order) }}" class="btn btn-white">Batal</a>
            <button type="submit" class="btn btn-whtie">Kirim Bukti
            </button>
          </div>
        </form>
      </div>
  ...
  <script>
    ...
    fileInput.addEventListener('change', function() {
        if (this.files.length > 0) {
            // Karena hanya satu file, kita ambil file pertama
