<x-app-layout>
  <x-slot name="header">
    <div class="flex items-center justify-between">
      <h2 class="text-2xl font-semibold text-gray-800">Upload Resep — Pesanan #{{ $order->id }}</h2>
      ...
      <div class="bg-white shadow-lg rounded-lg">
        <form method="POST" action="{{ route('shop.prescriptions.store',$order) }}" enctype="multipart/form-data">
          ...
          <div class="p-8 space-y-6">
            ...
            <div>
              <label for="file-upload" class="btn btn-white w-full h-32 flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50">
                <span class="flex items-center space-x-2">
                  ...
                  <span class="font-medium text-gray-600">
                    ...
                    <span class="text-cyan-600 underline">klik untuk memilih</span>
                  </span>
                </span>
                <span id="file-names-display" class="text-sm text-gray-500 mt-2"></span>
              </label>
          ...
          {{-- Tombol Aksi --}}
          <div class="bg-gray-50 px-6 py-6 flex items-center justify-end gap-4 rounded-b-lg">
            <a href="{{ route('shop.orders.show',$order) }}" class="btn btn-white">Batal</a>
            <button type="submit" class="btn btn-white">Kirim Resep
            </button>
          </div>
        </form>
      </div>

      {{-- Kartu Lampiran Terkini --}}
      @if($prescription && $prescription->attachments->count() > 0)
        <div class="bg-white shadow-lg rounded-lg p-6">
          <h3 class="text-xl font-semibold mb-4 border-b pb-3">Lampiran Terkirim</h3>
          <ul class="space-y-2">
            @foreach($prescription->attachments as $p)
              <li>
                <a href="{{ asset('storage/'.$p) }}" target="_blank" class="flex items-center gap-2 text-cyan-700 hover:underline">
