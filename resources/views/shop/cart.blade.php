    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="space-y-6">
                ...
                @endif
                {{-- Client-side error (hidden by default, animated) --}}
                    ...
                    <div class="bg-white dark:bg-slate-800 shadow-lg rounded-lg overflow-hidden">
                                    ...
                                    @foreach($items as $it)
                                        <tr data-stock="{{ $it['stock'] ?? 0 }}">
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                ...
                                                <div class="text-xs text-gray-500 dark:text-slate-400 mt-1">Stok: {{ $it['stock'] ?? 0 }}</div>
                                                @if($it['is_rx'])
                                                    <div class="text-xs text-red-600 dark:text-red-400">Perlu Resep</div>
                                                @endif
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <form method="POST" action="{{ route('shop.cart.update') }}" class="flex items-center gap-2 cart-update-form">
                                                    @csrf
                                                    <input type="hidden" name="id" value="{{ $it['id'] }}">
                                                    <input type="number" name="qty" value="{{ $it['qty'] }}" min="1" class="w-20 border-gray-300 rounded-md shadow-sm focus:ring-cyan-500 focus:border-cyan-500 text-sm cart-qty-input">
                        ...
                        <div class="bg-gray-50 dark:bg-slate-900 px-6 py-6 flex items-center justify-between">
                            ...
                            <div class="text-right">
                                ...
                                <a href="{{ route('shop.checkout.show') }}" id="checkout-button" class="btn btn-white">Lanjut ke Checkout</a>
                            </div>
                        </div>
                    </div>
                @endif
