...
<?php $component->withAttributes([]); ?>
     <?php $__env->slot('header', null, []); ?> 
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                <form action="<?php echo e(route('admin.analytics.lifecycle')); ?>" method="GET" class="flex flex-wrap items-center gap-2">
                    ...
                    <?php if(request('bulan') || request('quarter')): ?>
                        <a href="<?php echo e(route('admin.analytics.lifecycle')); ?>" class="h-10 px-4 inline-flex items-center gap-2 bg-red-50 dark:bg-red-900/20 text-red-600 rounded-xl text-sm font-medium hover:bg-red-100 border border-red-200 dark:border-red-800 transition-colors">Reset</a>
                    <?php endif; ?>
                </form>
                <a href="<?php echo e(route('admin.dashboard')); ?>" class="h-10 px-4 inline-flex items-center gap-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-600 dark:text-slate-300 text-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">&larr; Kembali</a>
            </div>
        </div>
     <?php $__env->endSlot(); ?>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-slate-600 dark:text-slate-400">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                            <?php $__empty_1 = true; $__currentLoopData = $courierTable; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30">
                                    <td class="px-6 py-4 font-bold text-slate-800 dark:text-white"><?php echo e($row->nama_kurir); ?></td>
                                    <td class="px-6 py-4 text-center"><?php echo e($row->total_order); ?></td>
                                    <td class="px-6 py-4 text-center">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300">
                                            <?php echo e(number_format($row->avg_duration, 1)); ?> Jam
                                        </span>
                                    </td>
                                </tr>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // --- CHART 2: KURIR (BAR) ---
        const courierData = <?php echo json_encode($courierChart, 15, 512) ?>;
...
<?php endif; ?><?php /**PATH C:\laragon\www\farmacheat\resources\views/admin/analytics/lifecycle.blade.php ENDPATH**/ ?>